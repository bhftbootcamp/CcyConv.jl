...
@testset "utils" begin
    @testset "get_currency" begin
        ...
        @testset "non-existent currency" begin
            ...
            @test_throws ErrorException("Currency not found") get_currency(my_graph, invalid_id)
        end

        @testset "empty graph" begin
            empty_graph = FXGraph()
            # Test with ID on empty graph
            @test_throws ErrorException("Currency not found") get_currency(empty_graph, UInt64(1))
        end
    end
