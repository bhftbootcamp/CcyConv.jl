...
function compute_path_value(fx::CcyConv.FXGraph, path::Vector{Int})::PathState
    ...
    for i = 1:length(path)-1
        ...
        elseif haskey(fx.edge_nodes, (v, u))
            ...
            reversed_price =
                CcyConv.Price(to_asset(price_obj), from_asset(price_obj), value)
...
function find_optimal_path(
    ...
    for path in all_paths
        ...
        if isnan(best_state.value) ||
           (use_max ? state.value > best_state.value : state.value < best_state.value)
            best_state = state
        end
    end
    ...
    for price_obj in best_state.chain
        push!(
            result,
            UInt64(fx.edge_encode[from_asset(price_obj)]) =>
...
function max_path_algorithm(fx::CcyConv.FXGraph, from_id::UInt64, to_id::UInt64)
    return find_optimal_path(fx, from_id, to_id, true)
end

function min_path_algorithm(fx::CcyConv.FXGraph, from_id::UInt64, to_id::UInt64)
    return find_optimal_path(fx, from_id, to_id, false)
end
...
function conv_max(
    ...
    return fx(CcyConv.MyCtx(), max_path_algorithm, from_asset, to_asset)
end
...
function conv_min(
    ...
    from_asset == to_asset && return CcyConv.ConvRate(from_asset, to_asset, 1.0)
    return fx(CcyConv.MyCtx(), min_path_algorithm, from_asset, to_asset)
end
